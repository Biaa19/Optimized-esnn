# -*- coding: utf-8 -*-
"""
Created on Fri Sep 18 09:56:06 2020
...
# SSA Function
def salp_swarm_algorithm(swarm_size = 5, min_values = [-5,-5], max_values = [5,5], iterations = 5, target_function = target_function):    
    count    = 0
...
ssa = salp_swarm_algorithm(swarm_size = 5, min_values = [-5,-5], max_values = [5,5], iterations = 10, target_function = six_hump_camel_back)

# Function to be Minimized (Rosenbrocks Valley). Solution ->  f(x) = 0; xi = 1
def rosenbrocks_valley(variables_values = [0,1]):
   func_value = 0
   last_x = variables_values[0]
   for i in range(1, len(variables_values)):
        func_value = func_value + (5 * math.pow((variables_values[i] - math.pow(last_x, 2)), 2)) + math.pow(1 - last_x, 2)
   return func_value

ssa = salp_swarm_algorithm(swarm_size = 5, min_values = [-0,-0], max_values = [1,1], iterations = 5, target_function = rosenbrocks_valley)
